#!/usr/bin/env python3
"""Listen to all MQTT messages for a device via WebSocket.

Connects to the staging broker and prints every incoming message to the
terminal in real time.  Runs indefinitely until interrupted with Ctrl+C.

Usage:
    python examples/ws_listen.py 2530294
    python examples/ws_listen.py 2530294 --raw        # hex only, no Helix decode
    python examples/ws_listen.py 2530294 --verbose     # enable debug logging
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Listen to all MQTT messages for a device via WebSocket.")
...
async def listen(device_serial: str, raw: bool) -> None:
    ...
    async with NextGenMQTTClient.staging() as client:
        async with client.connect_websocket(device_serial, use_secondary=False) as conn:
            print(f"Connected. Listening for messages on {device_serial} (Ctrl+C to stop)\n")
...
def main() -> None:
    ...
    try:
        asyncio.run(listen(args.device_serial, args.raw))
